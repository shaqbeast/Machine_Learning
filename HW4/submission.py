...
def create_zip_archive(
    patterns_to_zip: list[str], zip_filename: str, patterns_to_ignore=None
):
        ...
        for pattern in patterns_to_zip:
            ...
            for filepath in filepaths:
                if patterns_to_ignore:
                    if any(
                        glob.fnmatch.fnmatch(filepath, ignore_pattern)
                        for ignore_pattern in patterns_to_ignore
                    ):
...
print("\n\nDone!")
print(
    f"Please submit {submission_zip} to the programming assignment and the pdf to the non-programming assignment on Gradescope"
)
